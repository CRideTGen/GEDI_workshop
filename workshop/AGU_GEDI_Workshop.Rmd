...
gL1B_dir <- paste0(wd, '/workshop/data/GEDI01_B.002/')
gL2A_dir <- paste0(wd, '/workshop/data/GEDI02_A.002/')
gL2B_dir <- paste0(wd, '/workshop/data/GEDI02_B.002/')
shp_dir <- paste0(wd, '/workshop/data/shapefile/')
```

0.3: Source Scripts
```{r, warning=FALSE, message=FALSE}
#source rGEDI functions in scripts directory
source(paste0(wd, '/workshop/scripts/gedi_functions.R'))
```
...
if (fs::file_exists("gediLevel_clips.RData")) {
  load("gediLevel_clips.RData")
}else {
  ...
  save(list = c('gedilevel1B_clip', 'gedilevel2A_clip', 'gedilevel2B_clip'), file = "gediLevel_clips.RData")
}
...
###### using lowess filter ########
loe <- lowess(wf@dt$elevation, wf@dt$rxwaveform, 0.05)

wf_smooth@dt$elevation <- rev(loe$x)  #overwritring elevation values with smoothed values
wf_smooth@dt$rxwaveform <- rev(loe$y) #overwriting rxwaveform values with smoothed values
...
plot(wf, relative = FALSE, polygon = T, type = "l", lwd = 2, col = "forestgreen",
     xlab = "Waveform Amplitude", ylab = "Elevation (m)", ylim = c(ymin, ymax), xlim = qts + c(0, 0.1 * abs(diff(qts))))
# lines(wf_smooth@dt$rxwaveform, wf@dt$elevation, polygon=F, type='l', lwd = 3, col = 'red', add=TRUE)
